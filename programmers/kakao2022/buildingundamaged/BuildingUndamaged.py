...
def building_undamaged(board, skill):
    """
    Calculates the number of undamaged buildings after applying skill effects.

    Args:
        board: A 2D list representing the building's durability.
        skill: A list of skills to be applied.

    Returns:
        The number of buildings with durability greater than 0.
    """
    ...
    # Iterate through the skill list to apply each skill's effect.
    for type, r1, c1, r2, c2, degree in skill:
        # Determine the effect (negative for attack, positive for heal).
        effect = -degree if type == 1 else degree
    ...
    # Count the number of undamaged buildings.
    answer = 0
    for r in range(n):
        for c in range(m):
...
print(building_undamaged(board1, skill1))  # Output: 10